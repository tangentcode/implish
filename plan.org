...
2. ~refine[]~ - Parser/normalizer (exists but minimal - needs expansion)
3. ~eval[]~ - Interpreter (needs simplification)

** Key Insight
After refinement, everything should be in M-expression form: ~head[arg1; arg2; ...]~
...
* Transformation Rules (for refine[])
...
Source:  2 !
Load:    [INT(2), SYM(!)]
Refine:  [SYM(!) [INT(2)]]        ; list with head=!
Display: ![2]
Eval:    [INT(0), INT(1)]
#+end_example
...
Refine:  [SYM(*) [[SYM(+) [INT(2), INT(3)]], INT(5)]]
Display: *[+[2; 3]; 5]
Eval:    INT(25)     ; (2 + 3) * 5
#+end_example
...
Refine:  [SYM(+) [INT(2), [SYM(*) [INT(3), INT(5)]]]]
Display: +[2; *[3; 5]]
Eval:    INT(17)     ; 2 + (3 * 5)
#+end_example
...
Refine:  [LST [INT(1), INT(2), INT(3)]]
Display: 1 2 3
Eval:    [INT(1), INT(2), INT(3)]
#+end_example

Currently handled by ~refine[]~, should continue to work.
...
** Phase 2: Implement refine[] Transformations
