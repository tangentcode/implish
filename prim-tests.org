...
NOTE: Not yet implemented. Need to add reshape function that handles multi-dimensional arrays.
...
NOTE: Not yet implemented. Depends on reshape being implemented first.
...
NOTE: Not yet implemented. Need to add cut function.
...
NOTE: Partially implemented - works on numeric vectors but not mixed lists.
Issues: (1) Need 0N syntax for null integer (-2147483648), (2) Need bare backtick for null symbol,
(3) Current implementation only handles numbers, not general lists with different types.
...
NOTE: Implemented but cannot test. Same syntax issues as null test:
need 0N syntax for null integer and bare backtick for null symbol.
...
NOTE: Implemented but doesn't handle strings as character arrays.
The current implementation treats strings as atomic values, not character sequences.
Need to extend find to iterate over characters in strings.

** TODO find-dict : find-dict (dyadic ? with dict)
#+begin_src
...
NOTE: Not yet implemented. The find function needs to be extended to search dictionaries by value
and return the corresponding keys.
...
NOTE: Partially implemented - works for dictionaries but not general list indexing.
Need to extend at to handle lists and vectors.
...
NOTE: Not yet implemented. Need to add cast function for type conversions.
...
NOTE: Not yet implemented. Would need to call the implish evaluator on a string.

** TODO value-dict : value-dict (monadic .: for dict)
#+begin_src
> value map[`a `b; 11 22]
11 22
#+end_src
...
NOTE: Not yet implemented. Depends on value being implemented first.
