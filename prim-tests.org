...
** TEST map : map (dyadic ! for making dictionaries) :mismatch:
#+begin_src
> map[`a `b; 3 4]
[a:3; b:4]
> map[4 5; 6 7]
...
** TEST keys : keys (monadic !: for dictionaries) :mismatch:
#+begin_src
> keys [a:3; b:4]
`a `b
#+end_src
...
** TEST asc-dict : asc-dict (monadic <: for dictionaries) :mismatch:
#+begin_src
> asc [a:2; b:5; c:1]
`c `a `b
#+end_src
...
** TEST group : group (monadic =:) :mismatch:
#+begin_src
> group `c `a `b `b `a `c `a
[c: 0 5; a: 1 4 6; b: 2 3]
#+end_src
...
** TEST where-dict : where-dict (monadic &: for dictionaries) :mismatch:
#+begin_src
> where [`a `b `c ! 1 0 2]
`a `c `c
#+end_src
...
** TEST reverse-dict : reverse-dict (monadic |: for dictionaries) :mismatch:
#+begin_src
> reverse [a:3; b:4]
[b:4; a:3]
#+end_src
...
** TEST not-dict : not-dict (monadic ~: for dictionaries) :mismatch:
#+begin_src
> not [a: 2 0; b:1]
[a: 0 1; b:0]
#+end_src
...
** TEST concat-dict : concat-dict (dyadic , for dictionaries) :mismatch:
#+begin_src
...
** TEST count-dict : count-dict (monadic #: for dictionaries) :mismatch:
#+begin_src
> count [a:3; b:17]
2
#+end_src
...
** TEST take-dict : take-dict (dyadic # with list and dict) :mismatch:
#+begin_src
> take["fb"; "fab"!3 5 9]
"fb"!3 9
#+end_src
...
** TEST filter : filter (dyadic # with function)
#+begin_src
...
** TEST filter-dict : filter-dict (dyadic # with function and dict) :mismatch:
#+begin_src
...
** TEST drop-dict : drop-dict (dyadic _ with list and dict) :mismatch:
#+begin_src
...
** TEST filter-out : filter-out (dyadic _ with function)
#+begin_src
...
** TEST filter-out-dict : filter-out-dict (dyadic _ with function and dict) :mismatch:
#+begin_src
...
** TEST random-floats : random-floats (monadic ?: for numbers)
#+begin_src
> random-floats 6
...
** TEST find-dict : find-dict (dyadic ? with dict) :mismatch:
#+begin_src
...
** TEST random : random (dyadic ? with numbers)
#+begin_src
> random[5; 10]
0 3 3 7 7
> random[5; 10]
...
** TEST random-pick : random-pick (dyadic ? with list)
#+begin_src
> random-pick[8; "ABC"]
"ACBBCBCB"
#+end_src
...
** TEST random-chars : random-chars (dyadic ? with character)
#+begin_src
> random-chars[10; "A"]
"GHIJKLMNOP"
> random-chars[10; "0"]
...
** TEST random-distinct : random-distinct (dyadic ? with negative count)
#+begin_src
...
** TEST at-dict : at-dict (dyadic @ with dict) :mismatch:
#+begin_src
> at[[a:4; b:7]; `a]
4
> at[[a:4; b:7]; `q]
...
** TEST value-dict : value-dict (monadic .: for dict) :mismatch:
#+begin_src
> value [a:11; b:22]
11 22
#+end_src
...
** TEST value-func : value-func (monadic .: for function) :mismatch:
#+begin_src
> value {plus[x; y]}
...
** TEST dot-apply : dot-apply (dyadic .)
#+begin_src
...
** TEST bind : bind (dyadic . with function and dict) :mismatch:
#+begin_src
> f: bind[{a +:: x}; [a:100]]
{[x] a::. `a plus x}
> at[f; 5]
...
** TEST each-dyad : each-dyad (dyadic ')
#+begin_src
...
** TEST eachprior : eachprior (dyadic ':)
#+begin_src
...
** TEST eachprior-init : eachprior-init (dyadic ': with initial value)
#+begin_src
...
** TEST stencil : stencil (monadic ': with count)
#+begin_src
...
** TEST eachright : eachright (dyadic /:)
#+begin_src
...
** TEST over-init : over-init (dyadic / with initial value)
#+begin_src
> over[plus; 4; 5 2]
11
#+end_src
...
** TEST fixedpoint : fixedpoint (monadic /)
#+begin_src
...
** TEST for : for (monadic / with count)
#+begin_src
> for[enlist; 5; 7]
(((((7)))))
#+end_src
...
** TEST while : while (monadic / with condition)
#+begin_src
...
** TEST scan-init : scan-init (dyadic \ with initial value)
#+begin_src
> scan[times; 2; 3 2 9]
6 12 108
#+end_src
...
** TEST scan-fixedpoint : scan-fixedpoint (monadic \)
#+begin_src
...
** TEST scan-for : scan-for (monadic \ with count)
#+begin_src
> scan-for[enlist; 5; 7]
(7; (7); ((7)); (((7))); ((((7)))); (((((7))))))
#+end_src
...
** TEST scan-while : scan-while (monadic \ with condition)
#+begin_src
...
** TEST splice : splice (ternadic ?) :mismatch:
#+begin_src
> splice[1 2 3; 1 1; 4]
1 4 2 3
> splice["test"; 1 3; "u"]
...
** TEST splice-transform : splice-transform (ternadic ? with function) :mismatch:
#+begin_src
...
** TEST error-trap : error-trap (ternadic .) :mismatch:
#+begin_src
...
** TEST cond : cond (special $) :mismatch:
#+begin_src
> cond[1; "A"; 0; "B"; "C"]
"A"
> cond[0; "A"; 0; "B"; "C"]
