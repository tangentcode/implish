...
* DONE ok: niladic word that returns nil
#+name: ok
#+begin_src
> ok
#+end_src
...
* DONE set-word and get-word: syntactic sugar for set and get
#+name: setword-getword-basic
#+begin_src
> y: 99
99
> y
...
* DONE sequence: semicolon separates expressions
#+name: sequence
#+begin_src
> (x: 12; y: 34; z: 56)
56
...
* DONE sequence: syntactic sugar (whitespace-separated)
#+name: sequence-sugar
#+begin_src
> x: 12  y: 34  z: 56
56
...
* DONE branching: if-then-else with ite
#+name: branching-ite
#+begin_src
> ite[1; 42; 99]
42
...
* DONE repetition: while loop
#+name: repetition-while
#+begin_src
...
* DONE user-defined functions: as first-class values
#+name: user-fn-first-class
#+begin_src imp
> {x + 2}
{x + 2}
> {x * 2} 10
...
* DONE user-defined functions: assignment with curly braces
#+name: user-fn-assign-curly
#+begin_src imp
> f: {x + 2}
{x + 2}
> g: {x * 3}
...
* DONE user-defined functions: with infix operators
#+name: user-fn-infix-body
#+begin_src imp
> calc: {x + y * 2}
{x + y * 2}
> calc[3; 5]
...
* DONE imparse: GET symbol transformation
#+name: imparse-get
#+begin_src imp
> imparse load ":x"
get[`x]
...
* DONE imparse: SET symbol transformation
#+name: imparse-set
#+begin_src imp
> imparse load "x: 42"
set[`x ; 42]
...
* DONE imparse: SET with expression RHS
#+name: imparse-set-expr
#+begin_src imp
> imparse load "x: 2 + 3"
set[`x ; +[2 ; 3]]
...
* DONE imparse: chained SET
#+name: imparse-set-chained
#+begin_src imp
> imparse load "a: b: 99"
set[`a ; set[`b ; 99]]
...
* DONE imparse: postfix with multiple args (valence check)
#+name: imparse-postfix-valence
#+begin_src imp
> imparse load "2 ! 10"
![2 ; 10]
...
* DONE valence errors: too many arguments
#+name: valence-error
#+begin_src imp
> 2 ! 10
...
* DONE GET/SET: full integration test
#+name: get-set-integration
#+begin_src imp
> x: 42
42
> :x
...
* DONE dictionaries: basic creation and access
#+name: dict-basic
#+begin_src imp
> d: :[`a 1; `b 2 + 2]
:[`a 1; `b 4]
...
* DONE dictionaries: backtick infix access
#+name: dict-backtick-access
#+begin_src imp
> d: :[`a 1; `b 4]
:[`a 1; `b 4]
> d`a
...
* DONE dictionaries: multi-key access
#+name: dict-multi-key
#+begin_src imp
> d: :[`a 1; `b 2 + 2]
:[`a 1; `b 4]
...
Multiple keys can be accessed at once using space-separated strands. Returns a vector of values.

* DONE dictionaries: empty dictionary
#+name: dict-empty
#+begin_src imp
> e: :[]
:[]
...
* DONE dictionaries: nested values
#+name: dict-nested
#+begin_src imp
> d: :[`x [1 2 3]; `y "hello"]
:[`x [1 2 3]; `y "hello"]
...
* DONE dictionaries: symbol values with comma
#+name: dict-symbol-values
#+begin_src imp
> d: :[`c `d; `e `f `g]
:[`c, `d; `e, `f `g]
> d`c
